...
This monorepo also includes an experimental Codex version that runs as a
standalone Mattermost bot bridge. It lives in the sibling
[`codex-mattermost-bridge`](../codex-mattermost-bridge/) app, so its
`@openai/codex-sdk` dependency is never installed by the Claude plugin:

```bash
cd apps/codex-mattermost-bridge
bun install
bun codex-server.ts
```

See [`codex-mattermost-bridge/README.md`](../codex-mattermost-bridge/README.md)
for setup, pairing, and configuration.
